...
const source = readFileSync(resolve(dir, "HomePage.tsx"), "utf8");
...
describe("home page investor positioning", () => {
  ...
  it("keeps the support footer linked to the contact flow", () => {
    expect(source).toContain('["Contato", "/contato"]');
    expect(source).not.toContain("Central de ajuda");
  });
});
