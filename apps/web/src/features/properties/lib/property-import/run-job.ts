...
import { getPropertyExtractorBaseUrl } from "./enabled";
import { uploadImportedImages } from "./upload-images";
...
export async function runPropertyImportJob(jobId: string): Promise<void> {
  ...
  try {
    ...
    /** Margem antes do limite serverless (route maxDuration=300s). */
    const JOB_TIME_BUDGET_MS = 210_000;
    const jobStartedAt = Date.now();
    const isTimeBudgetExceeded = () => Date.now() - jobStartedAt > JOB_TIME_BUDGET_MS;
    ...
    for (const url of urls) {
      ...
      let siteDef: ReturnType<typeof detectSiteImportDefinition> = null;
      try {
        siteDef = detectSiteImportDefinition(new URL(url).hostname);
      } catch {
        siteDef = null;
      }
    ...
    await finalizeJob(
      okCount > 0 ? "completed" : "failed",
      okCount > 0 ? null : summarizeImportFailure(results),
      processed,
    );
  } catch (e) {
    const message = e instanceof Error ? e.message : "import_job_failed";
    await finalizeJob("failed", message, processed);
  } finally {
    if (!finalized) {
      const okCount = results.filter((r) => r.status === "ok").length;
      await finalizeJob(
        okCount > 0 ? "completed" : "failed",
        okCount > 0 ? null : "import_job_interrupted",
        processed,
      ).catch(() => undefined);
    }
  }
