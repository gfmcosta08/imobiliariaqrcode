...
import { detectSiteImportDefinition, mergeEnrichedListing } from "./registry";
...
export async function extractListingWithSiteParser(
  ...
  const siteDef = detectSiteImportDefinition(hostname);
  const parser = siteDef?.parser;
  ...
  let partial = parser.parse(html, url);

  if (siteDef.enrich) {
    partial = mergeEnrichedListing(partial, await siteDef.enrich(url, partial));
  }
