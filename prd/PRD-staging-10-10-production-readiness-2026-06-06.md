...
**Status atual:** parcial alto. Gates principais existem e agora usam `environment: staging`/`production`, validacao anti-producao e bypass de Vercel Authentication para Playwright. A branch foi publicada em PR draft (`https://github.com/gfmcosta08/imobiliariaqrcode/pull/2`). A tentativa real via API do GitHub em 2026-06-09 confirmou que a credencial local consegue ler o repo/criar PR, mas nao tem permissao admin para aplicar branch protection (`403` em `main`/`master`) nem alterar environments (`422`). Ainda falta dono/admin do repo aplicar ou liberar token admin para fechar a etapa sem ressalva.
...
- [ ] Validar/criar environments reais `staging` e `production` no GitHub com secrets e aprovacao de production. Tentativa API 2026-06-09: bloqueada por permissao/payload no GitHub (`422`).
- [ ] Tornar staging readiness obrigatorio no rito de promocao ou branch protection antes de producao. Tentativa API 2026-06-09: branch protection bloqueada por permissao admin insuficiente (`403`).
...
- [x] Etapa 4 concluida parcialmente alta: CI/CD tem gates principais, workflows usam environments separados, staging alias foi corrigido, branch/PR draft publicados e tentativa real via GitHub API registrada; falta permissao admin para aplicar branch protection e environments reais no GitHub.
