...
**Status atual:** parcial alto. Gates principais existem e agora usam `environment: staging`/`production`, validacao anti-producao e bypass de Vercel Authentication para Playwright. Ainda falta verificar/aplicar branch protection e environments reais no painel/API do GitHub.
...
- [ ] Validar/criar environments reais `staging` e `production` no GitHub com secrets e aprovacao de production.
- [ ] Tornar staging readiness obrigatorio no rito de promocao ou branch protection antes de producao.
...
- [x] Etapa 4 concluida parcialmente alta: CI/CD tem gates principais, workflows usam environments separados, staging alias foi corrigido e documentado; falta validar/aplicar branch protection e environments reais no GitHub.
